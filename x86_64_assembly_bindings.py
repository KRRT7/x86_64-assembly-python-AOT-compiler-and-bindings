...
class MemorySize(Enum):
    ...
    def to_ctype(self, signed: bool = False, py_type: type = int):
        if py_type is int:
            ...
            raise TypeError("Unsupported py_type provided.")

        return (
            {
                self.BYTE.value: (ctypes.c_ubyte, ctypes.c_byte),
                self.WORD.value: (ctypes.c_ushort, ctypes.c_short),
                self.DWORD.value: (ctypes.c_uint32, ctypes.c_int32),
                self.QWORD.value: (ctypes.c_uint64, ctypes.c_int64),
                self.DQWORD.value: (ctypes.c_uint64, ctypes.c_int64),
            }[self.value][signed]
            if py_type is int
            else ctypes.c_float
        )
