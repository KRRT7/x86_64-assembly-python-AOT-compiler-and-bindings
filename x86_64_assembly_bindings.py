...
class Program:
    ...
    def __str__(self) -> str:
        # Use separate comprehensions to avoid repeating isinstance checks
        global_functions = [f"global {fun}" for fun in self.functions]
        line_strings = []
        for i in self.lines:
            if isinstance(i, str):
                line_strings.append(i)
            else:
                indent = "    " if isinstance(i, Instruction) else ""
                line_strings.append(f"{indent}{i.write()}")
