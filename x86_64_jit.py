...
def load_floats(f, lines: list, ignore: bool = False, stack: Stack = None):
    ...
    if (
        ...
        return _load_float(f, lines, stack)
...
class StackFrame:
    ...
    def stack_offset(self):
        return sum(v.size.value // 8 for v in self.variables)
...
def _load_float(f, lines, stack):
    lines.append(" -- LOADING FLOAT")
    lines.append(
        Ins(
            "movq",
            ret_f := Reg.request_float(lines=lines, offset=stack.current.stack_offset),
            f,
        )
    )
    ret_f.meta_tags.add("float")
